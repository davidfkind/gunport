...
CX_PROBABILITY = float(0.0)     # (%) probability with which two individuals are crossed, range [0.0, 1.0)
MUTATION_RATE = float(0.25)     # (%) probability of mutating an individual, range [0.0, 1.0)
...
#    print("Best individual is %s, %s" % (best, best.fitness.values))   # debug
    ...
    if best.fitness.values[0] >= 100:
        ...
        dplobj = dpl.CPlot(rows, cols, "MA.partial", max_holes, max_dominoes)
